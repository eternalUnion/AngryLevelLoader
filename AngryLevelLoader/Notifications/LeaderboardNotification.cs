...
namespace AngryLevelLoader.Notifications
{
	internal class LeaderboardNotification : NotificationPanel.Notification
	{
		...
		private UserHistorySubNotification historyPanelNotification;
		...
		public override void OnUI(RectTransform panel)
		{
			try
			{
				...
				historyPanelNotification = new UserHistorySubNotification(currentUi.historyPanel);
				manageUserNotification = new ManageUserSubNotification(currentUi.manageUserPanel, historyPanelNotification);
				manageUserNotification.onExit += () => Reload(false);
								...
								SteamCacheManager.RequestUser(steamId, (user) =>
								{
									...
									entry.reportButton.onClick.AddListener(() => {
										if (AngryUser.hasLeaderboardPermissions)
											manageUserNotification.Show(steamId.ToString(), bundleGuid, levelId, category, actualDifficulty);
										else
										{
											AngryLeaderboards.RecordDifficulty reportDifficulty = difficulty;
											if (reportDifficulty == AngryLeaderboards.RecordDifficulty.ANY)
												Enum.TryParse(entryDifficulty, true, out reportDifficulty);

											ReportButton(steamId.ToString(), user.name, reportDifficulty);
										}
									});
								});
					...
					for (int i = 0; i < limit; i++)
					{
						...
						SteamCacheManager.RequestUser(steamId, (user) =>
						{
							...
							if (difficulty == AngryLeaderboards.RecordDifficulty.ANY)
								entry.username.text += $"\n<color=grey>{entryDifficulty}</color>";
							entry.profile.texture = user.profilePicture;
							entry.reportButton.onClick.AddListener(() => ReportButton(steamId.ToString(), user.name, reportDifficulty));
						});
		...
		private void ReportButton(string steamId, string username, AngryLeaderboards.RecordDifficulty reportDifficulty)
		{
			...
			currentUi.reportSend.onClick.AddListener(() => SendReport(steamId, username, reportDifficulty));

			currentUi.reportToggle.SetActive(true);
		}

		private void SendReport(string steamId, string username, AngryLeaderboards.RecordDifficulty reportDifficulty)
		{
			...
			var sendReportTask = AngryUser.ReportTask(AngryLeaderboards.RECORD_CATEGORY_DICT[dropdownCategories[currentUi.category.value]], AngryLeaderboards.RECORD_DIFFICULTY_DICT[reportDifficulty], bundleGuid, levelId, steamId, reason);
