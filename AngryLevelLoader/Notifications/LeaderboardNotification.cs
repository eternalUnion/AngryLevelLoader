...
namespace AngryLevelLoader.Notifications
{
	internal class LeaderboardNotification : NotificationPanel.Notification
	{
		...
		private async Task OpenHistoryWindow(string steamId)
		{
			...
			currentUi.historyPanel.bundleName.onEndEdit = new UnityEngine.UI.InputField.SubmitEvent();
			currentUi.historyPanel.levelName.onEndEdit = new UnityEngine.UI.InputField.SubmitEvent();
			...
			Dictionary<string, string> bundleGuidToName = history.bundleGuidPK
				...
				{
					var bundle = OnlineLevelsManager.catalog == null ? null : OnlineLevelsManager.catalog.Levels.Where(b => b.Guid == pair.Key).FirstOrDefault();
					if (bundle != null)
						return bundle.Name;

					bundle = OnlineCatalogManagerV2.Catalog == null ? null : OnlineCatalogManagerV2.Catalog.Levels.Where(b => b.Guid == pair.Key).FirstOrDefault();
					return bundle == null ? pair.Key : bundle.Name;
				});
			Dictionary<string, string> levelIdToName = history.levelIdPK
				.ToDictionary(pair => pair.Key, pair =>
				{
					var level = OnlineLevelsManager.catalog == null ? null : OnlineLevelsManager.catalog.Levels.Select(b => b.Levels).SelectMany(x => x).Where(l => l.LevelId == pair.Key).FirstOrDefault();
					if (level != null)
						return level.LevelName;

					level = OnlineCatalogManagerV2.Catalog == null ? null : OnlineCatalogManagerV2.Catalog.Levels.Select(b => b.Levels).SelectMany(x => x).Where(l => l.LevelId == pair.Key).FirstOrDefault();
			...
			*/
					...
					switch (currentUi.historyPanel.sortOrder.value)
					{
						...
						case 2:
							Dictionary<int, int> bundleIdToOrder = history.bundleGuidPK.Select(pair =>
							{
								...
								if (bundle == null)
									bundle = OnlineCatalogManagerV2.Catalog.Levels.Where(b => b.Guid == pair.Key).FirstOrDefault();
						...
						case 3:
							Dictionary<int, int> levelIdToOrder = history.levelIdPK.Select(pair =>
							{
								...
								if (level == null)
									level = OnlineCatalogManagerV2.Catalog.Levels.Select(b => b.Levels).SelectMany(x => x).Where(l => l.LevelId == pair.Key).FirstOrDefault();
			...
			void ReloadData()
			{
				...
				if (bundleSearchKeys.Length > 0)
				{
					...
					foreach (var bundle in OnlineLevelsManager.catalog.Levels.Concat(OnlineCatalogManagerV2.Catalog.Levels))
					{
			...
			currentUi.historyPanel.pageNumber.onEndEdit = new UnityEngine.UI.InputField.SubmitEvent();

			currentUi.historyPanel.nextPageButton.onClick.AddListener(() =>
			{
				pageNum = Clamp(pageNum + 1, 0, Math.Max(0, (filteredHistory.Count - 1) / entriesPerPage));
			...
			currentUi.historyPanel.pageNumber.onEndEdit.AddListener((e) =>
			{
				...
				pageNum = Clamp(newPageNum - 1, 0, Math.Max(0, (filteredHistory.Count - 1) / entriesPerPage));
				currentUi.historyPanel.pageNumber.SetTextWithoutNotify((pageNum + 1).ToString());
				RefreshList(false);
			});
